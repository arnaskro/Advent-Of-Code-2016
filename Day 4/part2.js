...
// [..{checksum:"peyac", id:977, name:"aczupnetwp-mfyyj-opalcexpye"}..]
function parseData(){
  // map each line...match into an array...typecast into hash of data
  input = input.map(line => (([, name, id, checksum]) => ({name, id: +id, checksum}))(line.match(/([a-z-]+)-(\d+)\[([a-z]+)\]/)));
}

function getFrequency(line, letter) {
  return line.filter(x => x === letter).length;
}

function getCommon(line) {
  // remove '-'' and split into array of letters
  line = line.replace(/-/g, '').split('');
  // remove common occurences by making it a set and converto it to an array
  var set = [...new Set(line)];
  // Sort by custom function...get top 5...join array data into string
  return set.sort((x, y) => getFrequency(line, y) - getFrequency(line, x) || x.localeCompare(y)).slice(0, 5);
}

function getRealData() {
  // filter lines where common 5 letters equals checksum
  input = input.filter(line => getCommon(line.name).join('') == line.checksum);
}
...
function decrypt() {
  input = input.map(line => [caesarShift(line.name, line.id), line.id ]);
}

function findNorthPole() {
  return input.find(line => line[0].includes('northpole'))[1];
}

parseData();
getRealData();
decrypt();
console.log(findNorthPole());